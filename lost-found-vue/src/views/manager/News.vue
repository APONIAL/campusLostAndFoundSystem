<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="content" label="内容" align="center">
        <template v-slot="scope">
          <el-button @click="showContent(scope.row.content)" plain type="success" size="mini">显示内容</el-button>
        </template>
      </el-table-column>
      <el-table-column prop="detail" label="详情页" align="center">
        <template v-slot="scope">
          <el-button @click="$router.push('/newsDetail?id='+scope.row.id)" plain type="info" size="mini">在详情页显示</el-button>
        </template>
      </el-table-column>
    ...
    <!--弹窗嵌套表单-->
    <el-dialog title="新闻" :visible.sync="formVisible"
               width="60%" @close="closeDialog" :close-on-click-modal="false">
  ...
  methods: {
    ...
    delBatch() {
      ...
      this.$confirm('是否要删除这些数据么?', '提示', {
        ...
        this.$request.delete('/news/del/batchByIds', {data: this.ids}).then(res => {
          if (res.code === '200') {
            this.$message({
              type: 'success',
              message: '删除成功!'
    ...
    saveOrUpdateNews() {
      ...
      this.$refs.formRef.validate(valid => {
        if (valid) {
          ...
          this.$request.post('/news/saveOrUpdate', this.form).then(res => {
            if (res.code === '200') {
